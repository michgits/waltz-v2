...
class MainClient extends Client {
    ...
    this.distube = new DisTube(client, {
        leaveOnEmpty: true, // Don't set this to "true" for 247 Commands working!
        emptyCooldown: 60,
        leaveOnFinish: true, // Don't set this to "true" for 247 Commands working!
        leaveOnStop: true,
        plugins: [
...
function checkSpotify(client) {
    if (client.config.SPOTIFY_TRACKS) {
        console.log("[INFO] you (enabled) spotify more tracks support!");
        return spotifyOn(client);
    } else {
        console.log("[INFO] you (not enabled) spotify more tracks support!");
        return spotifyOff();
    }
}
